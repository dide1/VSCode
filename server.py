...
@app.route("/")
def home():
    return render_template("pageHome.html", session=session.get('user'), pretty=json.dumps(session.get('user'), indent=4))


@app.route("/about", methods=["GET", "POST"])
def about():
        return render_template("pageAbout.html")
...
def sms_reply():
    ...
    if '-view'.lower() in msg: 
        for j in exp_price:
            for n in j: 
                n = int(n)
                total = total + n 

    if '/save: 50'.lower() in msg: 
        save = msg.split(':')[1]
        new_exp = Exp(exp_save = save)
        dbt.session.add(new_exp)
        dbt.session.commit()
        for j in exp_save:
            for n in j: 
